...
/** Represents the results of executing a query on an `<SUFMDatabase>`.
...
@interface SUFMResultSet : NSObject

@property (nonatomic, retain, nullable) SUFMDatabase *parentDB;
...
/** `SUFMStatement` used by result set. */

@property (atomic, retain, nullable) SUFMStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<SUFMStatement>`
 
 @param statement A `<SUFMStatement>` to be performed
 
 @param aDB A `<SUFMDatabase>` to be used
 
 @return A `SUFMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(SUFMStatement *)statement usingParentDatabase:(SUFMDatabase*)aDB;
