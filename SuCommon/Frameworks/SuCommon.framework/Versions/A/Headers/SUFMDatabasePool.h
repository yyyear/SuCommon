...
/** Pool of `<SUFMDatabase>` objects.
 ...
 @warning Before using `SUFMDatabasePool`, please consider using `<SUFMDatabaseQueue>` instead.

 If you really really really know what you're doing and `SUFMDatabasePool` is what
 you really really need (ie, you're using a read only database), OK you can use
 it.  But just be careful not to deadlock!

 For an example on deadlocking, search for:
 `ONLY_USE_THE_POOL_IF_YOU_ARE_DOING_READS_OTHERWISE_YOULL_DEADLOCK_USE_SUFMDATABASEQUEUE_INSTEAD`
 in the main.m file.
 */

@interface SUFMDatabasePool : NSObject
...
/** Create pool using path.
 ...
 @return The `SUFMDatabasePool` object. `nil` on error.
 */
...
/** Synchronously perform database operations in pool.
 ...
 @param block The code to be run on the `SUFMDatabasePool` pool.
 */
...
/** Synchronously perform database operations in pool using save point.
 ...
 @warning You can not nest these, since calling it will pull another database out of the pool and you'll get a deadlock. If you need to nest, use `<[SUFMDatabase startSavePointWithName:error:]>` instead.
*/
...
/** SUFMDatabasePool delegate category
 
 This is a category that defines the protocol for the SUFMDatabasePool delegate
 */

@interface NSObject (SUFMDatabasePoolDelegate)

/** Asks the delegate whether database should be added to the pool. 
 
 @param pool     The `SUFMDatabasePool` object.
 @param database The `SUFMDatabase` object.
 
 @return `YES` if it should add database to pool; `NO` if not.
 
 */

- (BOOL)databasePool:(SUFMDatabasePool*)pool shouldAddDatabaseToPool:(SUFMDatabase*)database;
...
- (void)databasePool:(SUFMDatabasePool*)pool didAddDatabase:(SUFMDatabase*)database;
